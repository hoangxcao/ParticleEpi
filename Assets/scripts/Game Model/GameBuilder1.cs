...
public class GameBuilder1 {
	...
	public void calculate(Initializer init){
		...
		int totalCuredDay = curedANum + curedBNum;
		int totalCuredTotal;
		if (calendar.Count == 1) {
		...
		int costNum = (sympTrtA * (int)init.getParam ("costA")) + (sympTrtB * (int)init.getParam ("costB")) + ((int) init.getParam("sickCost") * today.get ("totalSymps"));
		...
		today.restOfDay (sympTrtA, sympTrtB, sickTrtA, sickTrtB, curedANum, curedBNum, totalCuredDay, totalCuredTotal, catchDisease, costNum, totalCost, totalA, totalB, totalTrtA, totalTrtB, costA, costB, budget);
	}
